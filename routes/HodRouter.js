        ...
        try
        {
            ...
            else
            {
                if (typeOf(req.body.id) == 'String' || typeOf(req.body.codeOld) == 'String' || typeOf(req.body.codeNew) == "String")
                {
                    ...
                    const c1 = await courses.findOne(
                    {
                        code: courseCodeNew
                    });
                    if (c1 == null)
                    ...
                    {
                        courses: courseCodeNew
                    });
                    const dep1 = await department.findOne(
                    {
                        name: ufound.department
                    },
                    {
                        courses: courseCodeOld
                    });
...
HodRouter.route('/viewDaysOff/:memID')
    .get(async(req, res, next) =>
    {
        ...
        try
        {
            ...
            const amem = await academicMember.findOne(
            {
                Memberid: mem._id
            });
    ...
    .get(async(req, res, next) =>
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    a = await academicMember.findOne(
                    {
                        _id: dayoffreq[i].memberID
                    });
                    m = await members.findOne(
                    {
                        _id: a.Memberid
                    });
                    if (a.department == dep)
