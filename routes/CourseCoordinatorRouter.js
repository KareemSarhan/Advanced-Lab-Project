...
CourseCoordinatorRouter.route("/AcceptSlotLinkReq").post(
	async (req, res, next) => {
		// try {
...
CourseCoordinatorRouter.route("/RejectSlotLReq/:TAid").put(
	async (req, res, next) => {
		try {
			// console.log(req.params.TAid);
			const token = req.header("authtoken");
			//console.log(token)
			const DecodeToken = jwt_decode(token);
			const id = DecodeToken.id;
...
CourseCoordinatorRouter.route("/addSlot").post(async (req, res, next) => {
	try {
		//authenticate that this is a valid member
		//authorize that this is a CC member
		const payload = jwt.verify(req.header("authtoken"), key);
				...
				if (acMem.length != 0) {
					...
					if (acMem[0].type != "CourseCoordinator") {
						...
						if (req.body.type == null || req.body.type == "") {
							...
							if (l.length == 0) {
								return res.json({
									msg: "there does not exist a location with this name",
								});
							} else {
								//get the course id
...
CourseCoordinatorRouter.route("/updateSlot").post(async (req, res, next) => {
	try {
...
CourseCoordinatorRouter.route("/deleteSlot").delete(async (req, res, next) => {
	try {
		const SlotMember = req.body.SlotMember;
		const SlotTiming = req.body.SlotTiming;
