...
export class ViewSlotLinkingReq extends Component {
  ...
  componentDidMount() {
    axios.get('CC/viewSlotLinkReq',{headers:{"authtoken":localStorage.getItem("authtoken")}})
      .then(res => {
    ...
    render() {
        ...
        <table className="table" style={{
          ...
          {
            this.state.SLslotsReq.map((member)=>
            <Link to={"/RejectSlotLReq/"+member.memberID.Memberid.id }>Reject</Link> 
            )
            }
          </td>
